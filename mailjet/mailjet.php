...
if ($action == 'settitre' || $action == 'setemail_from' || $actino == 'setreplyto' || $action == 'setemail_errorsto') {
	...
	if ($action == 'settitre')
		...
		$mesg .= ($mesg ? '<br>' : '') . $langs->trans("ErrorFieldRequired", $langs->transnoentities("MailTitle"));
	else if ($action == 'setfrom' && empty($object->email_from))
		$mesg .= ($mesg ? '<br>' : '') . $langs->trans("ErrorFieldRequired", $langs->transnoentities("MailFrom"));
...
if (! empty($conf->global->MAILJET_ACTIVE) || ! empty($conf->global->MAILJET_ACTIVE_MAILING_ONLY)) {
	...
	if (! empty($mailjet->mailjet_id)) {
		...
		if (! empty($mailjet->mailjet_id) && $mailjet->getMailJetCampaignStatus(0) != $langs->trans('MailJetdraft') && $mailjet->getMailJetCampaignStatus(0) != $langs->trans('MailJetprogrammed')) {
			// Stats campaign mailjet
			print '<tr><td width="15%">';
			print $langs->trans("MailJetStatistics");
			print '</td><td colspan="">';
			// TODO : display statistic only when campagin is send
		...
		*/
			...
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatussent"),
					'data' => round($campaign_stats['delivered']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusbounce"),
					'data' => round($campaign_stats['bounce']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusblocked"),
					'data' => round($campaign_stats['blocked']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusspam"),
					'data' => round($campaign_stats['spam']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusqueued"),
					'data' => round($campaign_stats['queued']) 
			);
			...
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusopened"),
					'data' => round($campaign_stats['opened']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatusclicked"),
					'data' => round($campaign_stats['clicked']) 
			);
			$dataseries[] = array (
					'label' => $langs->trans("MailJetContactStatussent"),
					'data' => round($campaign_stats['delivered']) 
		...
		if ($mailjet->getMailJetCampaignStatus(0) == $langs->trans('MailJetdraft') || $mailjet->getMailJetCampaignStatus(0) == $langs->trans('MailJetprogrammed')) {
			if (! empty($mailjet->mailjet_id)) {
				if ($conf->global->MAILJET_API_VERSION == 1) {
					print '<a href="' . $mailjet->mailjet_url . '" target="_blank">MailJet</a>';
				} elseif ($conf->global->MAILJET_API_VERSION == 3) {
					print '<a href="https://app.mailjet.com/campaigns/edit/' . $mailjet->mailjet_id . '" target="_blank">MailJet</a>';
				}
			}
	...
	if (empty($mailjet->mailjet_permalink)) {
		dol_htmloutput_mesg($langs->trans("MailJetPermalinkMandatory"), '', 'error', 1);
	}
